...
```bash
node "/Users/rkreddy/Ramakrishna/projects/ed_conner_courses/ai_coder/finally/.claude/get-shit-done/bin/gsd-tools.cjs" validate health $REPAIR_FLAG
```
...
```bash
node "/Users/rkreddy/Ramakrishna/projects/ed_conner_courses/ai_coder/finally/.claude/get-shit-done/bin/gsd-tools.cjs" validate health
```
...
TASKS_DIR="/Users/rkreddy/Ramakrishna/projects/ed_conner_courses/ai_coder/finally/.claude/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$(find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null | wc -l)
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in /Users/rkreddy/Ramakrishna/projects/ed_conner_courses/ai_coder/finally/.claude/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf /Users/rkreddy/Ramakrishna/projects/ed_conner_courses/ai_coder/finally/.claude/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
